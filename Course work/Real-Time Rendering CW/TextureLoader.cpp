...
static GLuint newTextureFromFile(const wstring& textureFilePath, GLint _internalFormat = GL_RGBA) {
	...
	if (SUCCEEDED(hr)) {
		...
		glTexImage2D(GL_TEXTURE_2D, 0, _internalFormat, w, h, 0, GL_BGRA, GL_UNSIGNED_BYTE, buffer);
	}
...
GLuint TextureLoader::wicLoadTexture(const wstring& textureFilePath, const TextureGenProperties& textureProperties) {
	...
	GLuint newTexture = newTextureFromFile(textureFilePath, textureProperties.internalFormat);
...
GLuint TextureLoader::fiLoadTexture(const wstring& textureFilePath, const TextureGenProperties& textureProperties) {
	...
	glGenTextures(1, &newTexture);
	glBindTexture(GL_TEXTURE_2D, newTexture);
	glTexImage2D(GL_TEXTURE_2D, 0, textureProperties.internalFormat, w, h, 0, GL_BGR, GL_UNSIGNED_BYTE, buffer);
...
GLuint TextureLoader::loadCubeMapTexture(
	...
	const wstring& file_prefix,
	const wstring& file_extension,
	GLenum format,
	GLint minFilter,
	GLint maxFilter,
	...
	static const wstring cubeFaceName[6] = {

		wstring(L"_positive_x"),
		wstring(L"_negative_x"),
		wstring(L"_positive_y"),
		wstring(L"_negative_y"),
		wstring(L"_positive_z"),
		wstring(L"_negative_z")
	};
	...
	for (int i = 0; i<6; i++) {
		...
		HRESULT hr = loadWICBitmap(filename.c_str(), &textureBitmap);
