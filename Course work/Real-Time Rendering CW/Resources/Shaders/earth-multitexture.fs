...
uniform vec4        cameraPos; // to calculate specular lighting in world coordinate space, we need the location of the camera since the specular light
...
void main(void) {
    ...
    vec4 dayTexColour = texture2D(dayTexture, texCoord);
    ...
    vec3 E = cameraPos.xyz - posWorldCoord.xyz; // vector from point on object surface in world coords to camera
    E = normalize(E);
    ...
    vec3 clouds = texture2D(cloudMaskTexture, texCoord).rgb;

    vec3 dayTime = (diffuseColour + (specularColour * clouds.g)) * (1.0 - clouds.r) + clouds.r * lambertian;

    vec3 nightTime = texture2D(nightTexture, texCoord).rgb * (1.0 - clouds.r);
