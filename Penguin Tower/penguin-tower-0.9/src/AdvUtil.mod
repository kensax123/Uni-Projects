...
FROM AdvMap IMPORT Treasure, Rooms, DoorStatus,
                   MaxNoOfTreasures,
...
FROM AdvTreasure IMPORT ScatterTreasures ;
...
BEGIN
   ...
   WITH Treasure[t] DO
      REPEAT
         Rm := GetRandomRoom(NoOfRoomsToHidePlayers, ActualNoOfRooms) ;
         PositionInRoom(Rm, Xpos, Ypos, ok) ;
      UNTIL ok ;
      INCL(Rooms[Rm].Treasures, t)
   END ;
   ReleaseReadAccessToTreasure
END HideTreasure ;
