...
connectAttempts = 10
keyDict		= {}
hasPassedNameScreen = False
hasPassedMainScreen = False

#My modifications
def AltControls(key):
    global keyDict
    keyDict[K_w] = K_1	#move forward 1 space
    keyDict[K_a] = K_l	#turn left
    keyDict[K_d] = K_r	#turn right
    keyDict[K_s] = K_v	#vault turn
    keyDict[K_h] = K_h	#help
    keyDict[K_k] = K_f	#normal arrow
    keyDict[K_l] = K_m	#magic arrow
    keyDict[K_i] = K_p	#parry
    keyDict[K_o] = K_t	#thrust
    keyDict[K_p] = K_a	#attack
    keyDict[K_e] = K_o	#open door
    keyDict[K_r] = K_c	#close door
    keyDict[K_q] = K_e	#examine door
    keyDict[K_g] = K_g	#get treasure
    keyDict[K_z] = K_d	#drop treasure
    keyDict[K_x] = K_u	#use treasure
    keyDict[K_n] = K_s	#speak
    keyDict[K_m] = K_w	#write time
    keyDict[K_RETURN] = K_RETURN
    return keyDict[key]
...
def doArrows(font, background, name):
    text = font.render("Punches " + name, 1, White)
    textpos = text.get_rect()
